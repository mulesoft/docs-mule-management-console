...
You can also deploy, redeploy, or undeploy a deployment unit to or from a cluster programmatically using REST APIs. See xref:managing-deployments-using-rest-apis.adoc[Managing Deployments Using REST APIs].

== Deploying an Application To a Cluster

. If you have not yet created a cluster, then create a cluster as described in xref:creating-or-disbanding-a-cluster.adoc[Creating or Disbanding a Cluster].
...
For more information on managing application deployments, see xref:deploying-applications.adoc[Deploying Applications] and xref:maintaining-the-server-application-repository.adoc[Maintaining the Server Application Repository].

xref:creating-or-disbanding-a-cluster.adoc[<< Previous: *Creating or Disbanding a Cluster*]

xref:monitoring-a-cluster.adoc[Next: *Monitoring a Cluster* >>]