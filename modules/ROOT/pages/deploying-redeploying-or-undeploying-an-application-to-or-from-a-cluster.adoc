...
. If you have not yet created a cluster, then create a cluster as described in xref:3.7@creating-or-disbanding-a-cluster.adoc[Creating or Disbanding a Cluster].
...
For more information on managing application deployments, see xref:3.6@deploying-applications.adoc[Deploying Applications] and xref:3.6@maintaining-the-server-application-repository.adoc[Maintaining the Server Application Repository].