...
The Mule Management Console in a Web app designed to run in an application server or container, such as http://docs.mulesoft.com/tcat-server/v/7.1.0[Tcat Server] or https://www.mulesoft.com/tcat/understanding-apache-tomcat[Apache Tomcat], and usually packaged in a `.war` file.
...
(For instructions on setting up Mule for the first time, see https://docs.mulesoft.com/mule-user-guide/v/3.8/downloading-and-starting-mule-esb[Downloading Mule ESB].)
...
This documents assumes that you have Mule Enterprise 3.3.x already installed. Download it from the https://www.mulesoft.com/support-login[customer portal] if you need to upgrade this version. If you are using Mule Enterprise 3.4.x, please xref:3.4@installing-mmc.adoc[switch to the 3.4x version] of this documentation.

This document also assumes that you have Web application server such as Tcat, Tomcat, or JBoss installed. Please refer to the https://docs.mulesoft.com/mule-user-guide/v/3.7/hardware-and-software-requirements[Hardware and Software Requirements] for the latest information about hardware requirements and supported application servers, Java Runtime Environments, and browsers.
...
You can also deploy the Management Console agent in a xref:3.3@mule-runtime::deployment-scenarios.adoc[Mule ESB instance packaged as a Web app].
...
. Optionally, perform xref:installing-the-management-console.adoc[additional configuration] to use an external database, change the default folder for `mmc.data` or configure a different default port and server ID.
...
See http://docs.mulesoft.com/tcat-server/v/7.1.0[TCat Server Documentation] for instructions on how to deploy Web applications inside of Tcat.
...
To run the Management Console, make sure your container is running and the Management Console Web app has been deployed correctly. Then enter `http://localhost:8080/mmc` in your browser. (Note that if you are using the xref:installing-the-management-console.adoc[trial version], the default port is 8585 instead of 8080.) If you see the login screen (see below), you installed correctly and are now running the console.

image::mmc-login-alt.png[MMC_login]
...
See xref:persisting-environment-data.adoc[Persisting Environment Data] for instructions on how to set up the Management Console to use an external database for storing environment information, such as users, groups, applications, etc.

See xref:persisting-transaction-data.adoc[Persisting Transaction Data] for instructions on how to set up the Management Console to use an external database for storing transaction data (Business Events).
...
* Read the xref:quick-start-guide-to-mule-esb-server-and-the-management-console.adoc[Quick Start Guide to Mule ESB Server and the Management Console]
* Find out how to accomplish xref:common-tasks.adoc[Common Tasks] with the management console