...
There are a number of different types of alerts that you can set up. You indicate the alert type when you add or create a new alert (covered in xref:defining-slas-and-alerts.adoc[Defining SLAs and Alerts]). In addition to the type, you also select the severity level of the alert and, for most alert types, you define parameters for raising that alert.
...
|Custom Expression |Raises an alert depending on the evaluation of a target object custom expression a|
* Servers
* Groups

|Exception |Raises an alert when an exception of a specified type occurs for an application a|
* Servers
* Groups

|JMX Attribute |Raises an alert based on the evaluation of a JMX attribute value in an MBean a|
* Servers
* Groups
* Clusters

|Low Memory |Raises an alert when a specified memory pool reaches a specified memory threshold level a|
* Servers
* Groups

|Log Regex |Raises an alert based on a specified regular expression in specified log files a|
* Servers
* Groups
* Clusters
...
|Server Up |Raises an alert when a specified server or server group comes up. Also raises an alert when a node in a cluster comes up a|
* Servers
* Groups
* Clusters

|Thread Pool |Raises an alert when a specified thread pool reaches a specified threshold ratio a|
* Servers
* Groups

|URL Health |Raises an alert when a specified URL location returns a particular status code a|
* Servers
* Groups
* Clusters
...
image::alerts-unread-notice.png[]
