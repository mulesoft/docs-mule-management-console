...
+
The instructions on this page are valid _only_  for MMC 3.4.1. MMC 3.4.2 and later versions use a simpler configuration in which data persistence on MS SQL server is fully supported. If persisting data on MS SQL Server, it is recommended to use MMC 3.4.2 or later. For details, see the latest version of this document, xref:3.6@persisting-mmc-data-to-ms-sql-server.adoc[Persisting Data on MS SQL Server].
+
* This document lays out the instructions for deploying the Management Console on a production-ready environment using Apache Tomcat. All of the instructions should work with other xref:3.4@mule-runtime::hardware-and-software-requirements.adoc[supported application servers] as well, but this has not been tested.
...
* <<configuration-file-code-mmc-properties-code,mmc.properties>>
* <<configuration-file-code-applicationcontext-xml-code,applicationContext.xml>>
* <<configuration-file-code-jackrabbit-repo-xml-code,jackrabbit-repo.xml>>
* <<configuration-file-code-scheduler-properties-code,scheduler.properties>>
