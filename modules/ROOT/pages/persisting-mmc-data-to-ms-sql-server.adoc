...
[WARNING]
This document lays out the instructions for deploying the Management Console on a production-ready environment using Apache Tomcat. All of the instructions should work with other https://docs.mulesoft.com/mule-user-guide/v/3.7/hardware-and-software-requirements[supported application servers] as well, but this has not been tested.
...
[TIP]
The `spring.profiles.active` section in the `web.xml` configuration file allows you to define what external databases are used for storing environment and/or tracking data. For a quick instructions for all supported database servers, see xref:3.7@configuring-mmc-for-external-databases-quick-reference.adoc[Configuring MMC for External Databases - Quick Reference].
