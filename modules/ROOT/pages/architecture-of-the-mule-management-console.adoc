...
|image:mmc-6.png[MMC-6] |MMC's REST interface, which exposes MMC functionality through a REST API. MMC users can invoke this API to programmatically Mule instances paired with MMC. For more details about MMC's REST API, see . xref:rest-api-reference.adoc[MMC REST API].
|===
...
* Get familiar with the xref:rest-api-reference.adoc[MMC REST API].