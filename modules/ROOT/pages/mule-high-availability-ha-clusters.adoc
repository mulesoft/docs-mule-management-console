...
* Cluster lifecycle management and control. See xref:managing-mule-high-availability-ha-clusters.adoc[Managing Mule High Availability (HA) Clusters].
* Cluster and node performance monitoring. See xref:monitoring-a-cluster.adoc[Monitoring a Cluster].
* Raised alerts. You can set up an alert to appear when a node goes down and when a node comes back up. See xref:working-with-alerts.adoc[Working With Alerts].
...
* If your application processes messages from a non-transactional transport, use a xref:3.2@mule-runtime::reliability-patterns.adoc[reliability pattern] to move them to a transactional store such as a VM or JMS store.
...
* Implement xref:3.2@mule-runtime::reliability-patterns.adoc[reliability patterns] to create high reliability applications.
