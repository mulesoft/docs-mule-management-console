...
This document assumes that you are familiar with the basic xref:architecture-of-the-mule-management-console.adoc[Architecture of the Mule Management Console]. It also assumes that you have installed MMC version 3.5.0. (Download it from the http://www.mulesoft.com/support-login[customer portal] if you need to upgrade to this version.)
...
For more information on starting Mule with custom parameters, see https://docs.mulesoft.com/mule-user-guide/v/3.5/starting-and-stopping-mule-esb[Starting and Stopping Mule ESB].
====
...
* Learn how to xref:3.3@enabling-ldap-authentication.adoc[enable LDAP Authentication] for MMC