...
+
image::normal-cluster-status-green.png[normal_cluster_status_green]
+
...
You can also monitor the availability status of the nodes in a cluster by creating a server up or server down alert for a cluster. Create the alert as described in xref:defining-slas-and-alerts.adoc[Creating an Alert Definition] and select the cluster in the Server or Group field.
...
xref:deploying-redeploying-or-undeploying-an-application-to-or-from-a-cluster.adoc[<< Previous: *Deploying, Redeploying, or Undeploying an Application To or From a Cluster*]

xref:3.2@mule-runtime::reliability-patterns.adoc[Next: *Reliability Patterns* >>]