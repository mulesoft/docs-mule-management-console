...
*[mmc:Mule 3.2]* You can also use the Applications tab to to deploy, undeploy, and redeploy applications to a cluster of Mule ESB servers (nodes). For further details, see xref:deploying-redeploying-or-undeploying-an-application-to-or-from-a-cluster.adoc[Deploying, Redeploying, or Undeploying an Application To or From a Cluster].
...
xref:maintaining-the-server-application-repository.adoc[Next: *Maintaining the Server Application Repository* >>]