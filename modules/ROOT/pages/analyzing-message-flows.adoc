...
The Flow Analyzer tab enables you to see the cause of many problems and gives you insight into how to fix them. (Be sure that the flow you want to analyze is started and running. Use the Control Flows button from the Flows tab if you need to start a particular flow. See xref:3.7@tracking-flows.adoc[Tracking Flows] for more information on the Flows tab.)
...
See https://docs.mulesoft.com/mule-user-guide/v/3.5/routing-message-processors[Using Message Processors to Control Message Flow] to find out more about these standard message processors and how to use them to control message flow.
...
For more information on reading stream-based payloads, such as CXF streams, see how to apply an object-to-byte-array stream transformer in the section "Viewing Message Payload When Using Streams" in the xref:3.6@analyzing-flow-processing-and-payloads.adoc[Analyzing Flow Processing and Payloads] topic.
