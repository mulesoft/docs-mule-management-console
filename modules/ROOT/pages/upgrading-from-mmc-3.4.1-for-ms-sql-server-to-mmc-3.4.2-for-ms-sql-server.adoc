= Upgrading from MMC 3.4.1 for MS-SQL Server to MMC 3.4.2 for MS-SQL Server
...
MMC version 3.4.1 for MS-SQL Server allows you to xref:persisting-mmc-data-to-ms-sql-server.adoc[persist MMC data to MS SQL Server] instead of on MMC's default internal databases. Users of this version of MMC should upgrade to MMC 3.4.2 for MS-SQL Server.
...
[NOTE]
Before running MMC 3.4.2, you will also need to perform the steps described in xref:upgrading-to-mmc-3.4.2-with-an-external-database.adoc[Upgrading to MMC 3.4.2 with an External Database]. Additionally, if your current MMC installation uses LDAP for user authentication, you must complete the steps described in xref:upgrading-to-mmc-3.4.2-with-ldap.adoc[Upgrading to MMC 3.4.2 with LDAP].
...
* Read the configuration details for xref:persisting-mmc-3.4.2-data-to-ms-sql-server.adoc[Persisting MMC 3.4.2 Data to MS SQL Server].