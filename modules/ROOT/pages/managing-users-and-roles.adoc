...
You can also use LDAP to enable authentication (see xref:enabling-authentication-through-ldap.adoc[Enabling Authentication Through LDAP]). Note that after enabling authentication through LDAP, the Users link is no longer available.
