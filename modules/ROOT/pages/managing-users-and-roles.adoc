...
You can also use LDAP to enable authentication (see xref:enabling-ldap-authentication.adoc[Enabling LDAP Authentication]). Note, that after enabling authentication through LDAP, the Users link is no longer available.

See xref:automating-tasks-using-scripts.adoc[Automating Tasks Using Scripts] for an explanation of the Utility options (Admin Shell and Scheduler).
...
+
The default groups are Administrators, Deployers, Monitors, and Sever Administrators. If you need to create more groups or roles, see xref:managing-users-and-roles.adoc[Managing Groups] below.
...
The global permissions define the permissions that this group has regarding deployment, applications, alerts, flows, and so forth. See xref:managing-users-and-roles.adoc[Understanding Permissions] for more information.

=== Adding a User Group

. Click User Groups in navigation tree to see the existing user groups (roles).
+
image::user-groups-list.png[]
...
(Bear in mind that the permissions you can apply per server are currently limited. For details, see xref:3.7@managing-mmc-users-and-roles.adoc[Server Permissions Reference] below.)
...
|===
|
xref:setting-up-alert-destinations-and-notifications.adoc[<< Previous: Setting Up Alert Destinations and Notifications]
|
xref:enabling-ldap-authentication.adoc[Next: Enabling Authentication Through LDAP >>]
|===