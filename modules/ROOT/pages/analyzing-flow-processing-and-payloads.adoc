...
If you want to enable an MMC user to stop Flow Analyzer, assign the `Audit Flows - Admin Functions` permission to the user group that the user belongs. Refer to xref:managing-mmc-users-and-roles.adoc[Managing MMC Users and Roles] for details on assigning permissions.
...
* xref:tracking-flows.adoc[Tracking Flows] provides more information on using the Flows tab and setting up flow tracking, a prerequisite for flow analysis.
* xref:analyzing-message-flows.adoc[Analyzing Message Flows] takes you through using the Flow Analyzer panes in detail.
* xref:debugging-message-processing.adoc[Debugging Message Processing] illustrates the general steps for debugging an application's message processing.
* xref:debugging-the-loan-broker-example-application.adoc[Debugging the Loan Broker Example Application] shows how to use the console Flow Analyzer screen to find an application logic bug.
* https://docs.mulesoft.com/mule-user-guide/v/3.7/routers[Using Message Processors to Control Message Flow] provides reference information for the available Mule message processors or routers.