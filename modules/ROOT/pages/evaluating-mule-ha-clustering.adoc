...
. Start the three instances of the Mule server. See xref:quick-start-guide-to-mule-esb-server-and-the-management-console.adoc[Start the Mule Server] for instructions on how to start each server instance.
. Open the management console and login. See xref:quick-start-guide-to-mule-esb-server-and-the-management-console.adoc[Open the Console and Login] for instructions on how to do this.
. Register in the management console the two servers from which you removed the "mmc" application. (These are the instances that you want to include in the cluster.) The server that contains the "mmc" application will be automatically registered to the management console. See xref:quick-start-guide-to-mule-esb-server-and-the-management-console.adoc[Register the Server] for instructions.
...
You can also "disband" (that is, delete) one or more clusters. For further information about creating or disbanding a cluster, see xref:creating-or-disbanding-a-cluster.adoc[Creating or Disbanding a Cluster].
...
xref:quick-start-guide-to-mule-esb-server-and-the-management-console.adoc[<< Previous: *Quick Start Guide to Mule ESB Server and the Management Console*]

xref:setting-up-users.adoc[Next: *Setting Up Users* >>]