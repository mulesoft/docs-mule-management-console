...
*[Mule 3.2]* If you intend to evaluate Mule high availability clustering, see xref:evaluating-mule-ha-clustering.adoc[Evaluating Mule HA Clustering].
...
Click the Servers tab, and then click the Unregistered node in the left navigation pane. The window lists all servers that have come online and are not yet registered. Select the server to register, and then click the Register button. You do not have to specify the URL for the server since the auto-discovery feature does this for you. (for more information, see xref:working-with-mule-server-instances.adoc[Auto-Discovery for New Servers].)

You can also manually register a server. To do so, click the Servers tab, then the New Server button. In the dialog that opens, enter a name for the server and the URL address for the server. (See xref:working-with-mule-server-instances.adoc[Manually Registering a Server].)
...
image::hello-example-folder2.png[]
...
When you view the other sample application folders, notice that the sets of files are similar to the hello application folder. If you are just getting started, you may want to try using the hello application, since it is the simplest example.
...
image::deploy-to-cluster.png[deploy_to_cluster]

You can find out more about working with clusters xref:managing-mule-high-availability-ha-clusters.adoc[here].
...
image::custom-event-discount.png[]
...
You can find out more about working with the Business Events tab xref:analyzing-business-events.adoc[here].

== Where To Go From Here

Setting alerts (SLAs) for an application are another important task you do through the console. xref:adding-and-managing-alerts-and-slas.adoc[Adding and Managing Alerts and SLAs] shows you how to get started with alerts.

xref:how-to-use-the-management-console.adoc[<< Previous: *How to Use the Management Console*]

xref:evaluating-mule-ha-clustering.adoc[Next: *Evaluating Mule HA Clustering* >>]