...
A field for which data is required is marked with an exclamation point image:warning.png[] and highlighted in red if left blank. (The figure below has a field marked as required.) For more information on these fields, see mmc:below.
...
Custom alerts are executed as Groovy script snippets. The execution context is exactly the same as for other scripts in Mule. The context bindings for these scripts are described in xref:3.2@mule-runtime::scripting-module-reference.adoc#script-context-bindings[Script Context Bindings].
...
xref:working-with-alerts.adoc[<< Previous: *Working With Alerts*]

xref:setting-up-alert-destinations-and-notifications.adoc[Next: *Setting Up Alert Destinations and Notifications* >>]