...
Custom alerts are executed as Groovy script snippets. The execution context is exactly the same as for other scripts in Mule. The context bindings for these scripts are described in xref:3.3@mule-runtime::scripting-module-reference.adoc#script-context-bindings[Script Context Bindings].
