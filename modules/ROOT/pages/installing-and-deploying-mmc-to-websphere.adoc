...
This document describes how to install the production version of Mule Management Console to WebSphere. Note that you can also deploy Mule to a variety of other application servers or containers, such as Tomcat or JBoss. See xref:3.6@installing-the-production-version-of-mmc.adoc[Installing the Production Version of MMC] for full details.
...
* MMC version 3.4.X prior to 3.4.2: xref:3.7@persisting-mmc-data-on-external-databases.adoc[Persisting MMC Data On External Databases]
* MMC version 3.4.2 and above: xref:3.7@persisting-mmc-data-on-external-databases.adoc[Persisting MMC Data On External Databases]
...
[NOTE]
====
`sqlserver.jar` is the ConnectJDBC type 4 driver for Microsoft SQL Server provided by IBM. If you use `sqlserver.jar` as described above, you do not need to use the JDBC driver provided by Microsoft (`sqljdbc4.jar`). For details, see xref:3.6@persisting-mmc-data-to-ms-sql-server.adoc[Persisting MMC Data to MS SQL Server] and the IBM documentation for http://www-01.ibm.com/support/knowledgecenter/SS7JFU_7.0.0/com.ibm.websphere.express.doc/info/exp/ae/rdat_minreqmssql.html[data source minimum required settings for Microsoft SQL Server].
====
|===

== See Also

* Get familiar with the xref:3.7@orientation-to-the-console.adoc[MMC console].
* Learn the basics of using MMC with the xref:3.6@mmc-walkthrough.adoc[MMC Walkthrough].
* Learn more about how to xref:3.7@setting-up-mmc.adoc[set up MMC] to meet your needs.
* Access the xref:3.6@troubleshooting-with-mmc.adoc[troubleshooting] guide.