...
The management console LDAP support JAR file contains the necessary classes for the management console to authenticate against an LDAP server. You can download the link:{attachmentsdir}/galaxy-ldap-2.2.2.jar[galaxy-ldap-2.2.2.jar] file and store it in the `/WEB-INF/lib` directory under the MMC web application (`<TomcatHome>/webapps/console/WEB-INF/lib`).
...
xref:validating-the-license.adoc[<< Previous: *Validating the License*]

xref:customizing-the-dashboard.adoc[Next: *Customizing the Dashboard* >>]