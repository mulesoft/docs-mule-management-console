...
[TIP]
This page applies to version 3.3 of the Mule Management Console, which we refer to throughout the document as "the console", or "the Mule console". For information on enabling LDAP on version 3.2.x of the console, click http://docs.mulesoft.com/tcat-server/v/7.1.0/integrating-with-ldap[here].

[NOTE]
The Mule Management Console provides REST APIs for xref:using-the-management-console-api.adoc[programatic access to much of the console's functionality], including cluster management, applications, flows and deployments. You can also use the REST API to configure xref:using-the-management-console-api.adoc[automated replication of LDAP user groups].
...
The last section of this document provides an xref:setting-up-and-managing-users-via-ldap.adoc[example] detailing this configuration.

To configure the console to authenticate users via LDAP, perform the following high-level tasks:

* xref:setting-up-and-managing-users-via-ldap.adoc[Obtain LDAP parameters]
* xref:setting-up-and-managing-users-via-ldap.adoc[Set up users and groups on LDAP]
* xref:setting-up-and-managing-users-via-ldap.adoc[Enable the LDAP Spring profile]
* xref:setting-up-and-managing-users-via-ldap.adoc[Enable LDAP on the console]
...
. xref:setting-up-and-managing-users-via-ldap.adoc[Enabling the LDAP Spring profile]
. xref:setting-up-and-managing-users-via-ldap.adoc[Creating the LDAP Configuration file]
...
. Stop the console
. Enable LDAP on the console (see instructions xref:setting-up-and-managing-users-via-ldap.adoc[above])
...
* xref:setting-up-and-managing-users-via-ldap.adoc[Download and install the OpenLDAP server and utilities]
* xref:setting-up-and-managing-users-via-ldap.adoc[Download and install the Apache Directory Studio LDAP browser]
* xref:setting-up-and-managing-users-via-ldap.adoc[Configure the LDAP server and create the database]
* xref:setting-up-and-managing-users-via-ldap.adoc[Create LDAP users and groups using Apache Directory Studio] (or alternatively, xref:setting-up-and-managing-users-via-ldap.adoc[create the users and groups by importing an LDIF file])
* xref:setting-up-and-managing-users-via-ldap.adoc[Configure the console for use with LDAP]
...
. xref:setting-up-and-managing-users-via-ldap.adoc[Stop the slapd daemon]
. xref:setting-up-and-managing-users-via-ldap.adoc[Modify slapd's default parameters file, `/etc/default/slapd`]
. xref:setting-up-and-managing-users-via-ldap.adoc[Modify the system-wide LDAP configuration file, `/etc/ldap/ldap.conf`]
. xref:setting-up-and-managing-users-via-ldap.adoc[Restart the slapd daemon]
...
As shown xref:setting-up-and-managing-users-via-ldap.adoc[above], the LDAP configuration file stores the user name and password for the root user of the database. The password can be stored in clear text; however, this constitutes a security risk, since the LDAP configuration file is world-readable. To generate a hash for the password, run the following command: `slappasswd -h <scheme> -s <secret>`
...
xref:setting-up-and-managing-users-via-ldap.adoc[Populating the LDAP database using a GUI (Apache Studio)]
xref:setting-up-and-managing-users-via-ldap.adoc[Populating the LDAP database by importing an LDIF file]
...
image::2-auth-user.png[2-auth_user]
...
image::4-initial-db-objects.png[4-initial_DB_objects]
...
+
image::5-create-obj-wizard-ob-classes.png[5-create_obj_wizard-ob_classes]
...
+
image::6-create-obj-wizard-ob-classes.png[6-create_obj_wizard-ob_classes]

. Click *OrganizationalUnit*.
. Click *Add*, then click *Next*. The next window is the *Distinguished Name* window:
+
image::7-create-obj-wizard-dn.png[7-create_obj_wizard-DN]
...
+
image::8-create-obj-wizard-attrs.png[8-create_obj_wizard-attrs]
+
...
+
image::9-create-obj-wizard-user-attrs.png[9-create_obj_wizard-user_attrs]
...
+
image::10-create-obj-wizard-user-pass.png[10-create_obj_wizard-user_pass]
...
image::11-create-obj-wizard-groupmember.png[11-create_obj_wizard-groupmember]
...
Follow the procedure outlined xref:setting-up-and-managing-users-via-ldap.adoc[above] to Enable LDAP on the console. For this example, the contents of the `mmc-ldap.properties` file are as follows:
...
image::13-mmc-limited-dashboard.png[13-mmc-limited_dashboard]
