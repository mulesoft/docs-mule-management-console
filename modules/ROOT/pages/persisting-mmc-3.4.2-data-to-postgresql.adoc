...
[TIP]
The `spring.profiles.active` section in the `web.xml` configuration file allows you to define what external databases are used for storing environment and/or tracking data. For a quick instructions for all supported database servers, see xref:configuring-mmc-3.4.2-for-external-databases-quick-reference.adoc[Configuring MMC 3.4.2 for External Databases - Quick Reference].
