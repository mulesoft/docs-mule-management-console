...
This document, as well as the rest of the documents that make up the xref:rest-api-reference.adoc[Mule Management Console REST API Reference Guide], are a technical reference only. This document does not provide contextual information such as instructions, use cases or scenarios. To understand this document, you should be familiar with https://www.mulesoft.com/resources/esb/what-mule-esb[MULE ESB], Mule Servers and the xref:using-the-management-console-api.adoc[REST API].
...
For a detailed description of the format used in this document, consult xref:using-the-management-console-api.adoc[Content Organization and Conventions].
...
Note that this syntax is an example. Be sure to replace `+http://localhost:8585/mmc-console-3.7.3+` with your own MMC hostname, port number, and setting for mmc-console-3.7.3 (which is usually the name of the deployed mmc .war file).
