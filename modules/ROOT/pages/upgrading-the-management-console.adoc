...
*Note*: MMC is downloaded separately from Mule. Download MMC from the Support Portal.
...
* If managing MMC users via LDAP, edit the MMC config files to include LDAP information such as host, connection parameters, etc. See https://docs.archive.mulesoft.com/mule-management-console/v/3.4/setting-up-and-managing-users-via-ldap[Setting Up and Managing Users via LDAP] for details

|*3.5.x and above* a|
* If persisting MMC data on an external database, copy the MMC config files containing database connection information to your new installation. See xref:persisting-mmc-data-on-external-databases.adoc[Persisting MMC Data On External Databases] for details
* If managing MMC users via LDAP, copy the MMC config files containing the LDAP information. See xref:setting-up-and-managing-users-via-ldap.adoc[Setting Up and Managing Users via LDAP] for details

|*Earlier than 3.4.x* a|
* Follow the steps required to upgrade your MMC agent
* If using an external database, follow the steps required to xref:upgrading-to-mmc-with-an-external-database.adoc[upgrade to MMC with an external database]
...
* If managing MMC users via LDAP, migrate your MMC LDAP configuration as described in xref:upgrading-to-mmc-with-ldap.adoc[Upgrading to MMC with LDAP]
...
* Follow the steps required to xref:upgrading-from-mmc-3.4.1-for-ms-sql-server-to-latest-mmc-for-ms-sql-server.adoc[upgrade from MMC 3.4.1 for MS-SQL Server to the latest MMC for MS-SQL Server]
* If managing MMC users via LDAP, migrate your MMC LDAP configuration as described in xref:upgrading-to-mmc-with-ldap.adoc[Upgrading to MMC with LDAP]
...
Migrating your config files is described in xref:upgrading-to-mmc-with-an-external-database.adoc[Upgrading to MMC with an External Database]. The process is the same for upgrading to 3.4.2, for 3.5.x and later versions.

After migrating your config files, you need to run migration scripts on your target MS SQL database. This process is described in xref:upgrading-from-mmc-3.4.1-for-ms-sql-server-to-latest-mmc-for-ms-sql-server.adoc[Upgrading from MMC 3.4.1 for MS-SQL Server to the latest MMC for MS-SQL Server].

Finally, if you are managing MMC users via LDAP, you need to migrate your MMC LDAP configuration as described in xref:upgrading-to-mmc-with-ldap.adoc[Upgrading to MMC with LDAP].

The complete sequence of steps is:

. Download the latest version of MMC.
. Stop your MMC 3.4.1 for MS SQL server.
. In the new MMC, modify the configuration for external data persistence as described in xref:upgrading-to-mmc-with-an-external-database.adoc[Upgrading to MMC with an External Database].
. If managing MMC users via LDAP, modify the configuration for LDAP as described in xref:upgrading-to-mmc-with-ldap.adoc[Upgrading to MMC with LDAP]. If not managing MMC users via LDAP, skip this step.
. On your target database on MS SQL server, run the SQL migration scripts as described in xref:upgrading-from-mmc-3.4.1-for-ms-sql-server-to-latest-mmc-for-ms-sql-server.adoc[Upgrading from MMC 3.4.1 for MS-SQL Server to the latest MMC for MS-SQL Server].
...
You must run this the migration script _before_ running the latest MMC for the first time. For instructions and the SQL scripts to run, see xref:migration-scripts-for-business-events.adoc[Migration Scripts for Business Events].
