...
This document assumes that you are familiar with the basic xref:3.6@architecture-of-the-mule-management-console.adoc[Architecture of the Mule Management Console], and that you have xref:3.7@installing-mmc.adoc[installed] either the trial or production version of the Management Console.
...
When you run the xref:3.6@installing-the-production-version-of-mmc.adoc[production version] of MMC for the first time, it will attempt to detect any Mule ESB servers on the network, but will not try to pair with any server it finds. You must manually register your Mule ESB server.
...
[NOTE]
====
If the server is part of a cluster, you must disband the cluster before you unregister the server. See xref:3.7@creating-or-disbanding-a-cluster.adoc[Creating or Disbanding a Cluster] for details.
====
...
* Learn how to xref:3.7@managing-mule-servers-clusters-and-groups.adoc[stop or restart] a Mule server or cluster via MMC.
* Use MMC to organize servers and clusters into xref:3.7@managing-mule-servers-clusters-and-groups.adoc[groups].
* Create and manage xref:3.7@creating-or-disbanding-a-cluster.adoc[High Availability (HA) clusters] of Mule servers.
* Learn how to xref:3.6@deploying-applications.adoc[deploy applications] to Mule servers.