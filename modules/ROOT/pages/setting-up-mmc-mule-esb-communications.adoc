...
This document assumes that you are familiar with the basic xref:architecture-of-the-mule-management-console.adoc[Architecture of the Mule Management Console], and that you have xref:installing-mmc.adoc[installed] either the trial or production version of the Management Console.
...
When you run the xref:installing-the-production-version-of-mmc.adoc[production version] of MMC for the first time, it will attempt to detect any Mule ESB servers on the network, but will not try to pair with any server it finds. You must manually register your Mule ESB server.
