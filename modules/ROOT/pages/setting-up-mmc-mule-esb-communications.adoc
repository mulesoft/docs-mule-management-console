= Setting Up MMC-Mule Communications
...
* How to register or unregister a Mule server in MMC
...
This document assumes that you are familiar with the basic xref:architecture-of-the-mule-management-console.adoc[Architecture of the Mule Management Console], and that you have  the Management Console.
...
Each Mule server can only be registered with one MMC, but each MMC can manage an unlimited number of Mule ESB servers.

When you run the xref:installing-the-production-version-of-mmc.adoc[production version] of MMC for the first time, it will attempt to detect any Mule  servers on the network, but will not try to pair with any server it finds. You must manually register your Mule  server.

To pair the MMC with a Mule instance, you provide MMC with the URL of the Mule agent listening on the the Mule ESB server. The default port is 7777, and the path is `mmc-support`. So, if you wish to register a Mule  server on host 16.172.1.25, the URL would be `+http://16.172.1.25:7777/mmc-support+`. When you provide this URL to MMC, the Management Console will attempt to pair with the Mule agent in the specified URL, then report success or failure.

To register a Mule  server to MMC, follow these steps:
...
+
image::unreg-servers.png[unreg_servers]
...
+
image::unreg-servers-list.png[unreg_servers_list]
...
+
image::add-server-manually.png[add_server_manually]
...
image::reg-servers-list.png[reg_servers_list]

== Registering a Mule  Server to a New MMC
...
=== Unregistering a Mule  Server

[NOTE]
====
If the server is part of a cluster, you must disband the cluster before you unregister the server. See xref:creating-or-disbanding-a-cluster.adoc[Creating or Disbanding a Cluster] for details.
====
...
After you delete the file, you can register the Mule  server to any instance of MMC, following the steps outlined <<Registering Mule ESB Servers with MMC>>. When you register the server, a new `truststore.jks` will be created.
...
[TIP]
====
If you wish to include a port range, letting Mule  bind to the first available port, use `<low port>-<high port>`, for example, `7780-7785`.
====
...
If MMC is running as an app deployed by the Mule  server it is bundled with. By default it listens on port 8585. To modify MMC's listening port, follow the steps below.

. Ensure that the Mule  server is not running.
...
* Learn how to xref:managing-mule-servers-clusters-and-groups.adoc[stop or restart] a Mule server or cluster via MMC.
* Use MMC to organize servers and clusters into xref:managing-mule-servers-clusters-and-groups.adoc[groups].
* Create and manage xref:creating-or-disbanding-a-cluster.adoc[High Availability (HA) clusters] of Mule servers.
* Learn how to xref:deploying-applications.adoc[deploy applications] to Mule servers.