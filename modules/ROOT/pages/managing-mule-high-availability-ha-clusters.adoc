...
[NOTE]
For an overview of clusters, including benefits and best practices, see https://docs.mulesoft.com/mule-user-guide/v/3.6/mule-high-availability-ha-clusters[Mule High Availability HA Clusters].

You can perform the following cluster-related tasks with the console:

* xref:3.7@creating-or-disbanding-a-cluster.adoc[Create a cluster]
* xref:3.6@deploying-redeploying-or-undeploying-an-application-to-or-from-a-cluster.adoc[Deploy, redeploy, or undeploy an application to or from a cluster]
* xref:3.6@monitoring-a-cluster.adoc[Monitor a cluster, including its status]
* xref:3.7@creating-or-disbanding-a-cluster.adoc["Disband" or delete a cluster]
