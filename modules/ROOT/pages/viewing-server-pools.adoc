...
The Pools tab displays information about the thread pools used by this Mule ESB instance. For information on configuring thread pools in Mule, see xref:3.2@mule-runtime::tuning-performance.adoc[Tuning Performance] in the Mule Runtime documentation.
...
xref:viewing-threads.adoc[<< Previous: *Viewing Threads*]

xref:working-with-the-server-configuration-files.adoc[Next: *Working with the Server Configuration Files* >>]