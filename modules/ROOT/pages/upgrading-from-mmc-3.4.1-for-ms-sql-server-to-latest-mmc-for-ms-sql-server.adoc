...
MMC version 3.4.1 for MS-SQL Server allows you to xref:persisting-mmc-data-to-ms-sql-server.adoc[persist MMC data to MS SQL Server] instead of on MMC's default internal databases. Users of this version of MMC should upgrade to the latest version of MMC, which includes support for MS-SQL Server.
...
[NOTE]
Before running the latest version of MMC, you will also need to perform the steps described in xref:upgrading-to-mmc-with-an-external-database.adoc[Upgrading to MMC with an External Database]. Additionally, if your current MMC installation uses LDAP for user authentication, you must complete the steps described in xref:upgrading-to-mmc-with-ldap.adoc[Upgrading to MMC with LDAP].
...
* Read the configuration details for xref:persisting-mmc-data-to-ms-sql-server.adoc[Persisting MMC Data to MS SQL Server].