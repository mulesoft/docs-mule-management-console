...
This document, as well as the rest of the documents that make up the xref:rest-api-reference.adoc[Mule Management Console REST API Reference Guide], are a technical reference only. This document does not provide contextual information such as instructions, use cases or scenarios. To understand this document, you should be familiar with xref:3.4@mule-runtime::index.adoc[MULE ESB], Clusters and the xref:using-the-management-console-api.adoc[REST API].
