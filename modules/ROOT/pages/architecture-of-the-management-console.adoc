...
The management console server is a web application packaged as a WAR file that executes from within your web container. As such, it must be correctly deployed on the container, such as by placing the WAR file in the proper location within your web container installation. For example, for an Apache Tomcat installation, you deploy the mule management console WAR (`mmc.war`) file using https://www.mulesoft.com/tcat/download[MuleSoft Tcat Server], or by copying it to the Tomcat `/webapps` directory.
...
xref:installing-the-management-console.adoc[Next: *Installing the Management Console* >>]
