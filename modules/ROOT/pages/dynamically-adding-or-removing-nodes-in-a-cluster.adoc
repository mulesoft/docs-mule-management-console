...
From the Management Console or the Console's xref:3.7@rest-api-reference.adoc[REST API] you can dynamically add or remove nodes to or from a cluster without disbanding it.

This page details the procedure to add or remove a node in the Management Console. For details on performing these operations using the REST API, consult the xref:3.7@clusters.adoc[REST API Clusters] documentation.
...
To summarize, a node that join a cluster which has no alerts configuration, will retain its alerts configuration (if any). A node that join a cluster which has alerts configuration, will have their alerts configuration (if any) overridden by that of the cluster, but will recover its alerts if and when is ceases to be part of the cluster.
