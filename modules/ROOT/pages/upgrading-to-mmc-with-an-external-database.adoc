= Upgrading to MMC with an External Database
...
[NOTE]
If your current installation of MMC xref:setting-up-and-managing-users-via-ldap.adoc[uses LDAP for managing users], you also need to migrate your LDAP configuration. See xref:upgrading-to-mmc-with-ldap.adoc[Upgrading to MMC with LDAP] for details.
...
----
+
In the example above, both the workspace name and the default workspace name are mmcworkspace.
+
...
* Learn how to xref:upgrading-to-mmc-with-ldap.adoc[upgrade to MMC with LDAP].
