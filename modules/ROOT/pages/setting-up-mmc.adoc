...
To set up MMC, you first need to register your Mule servers with MMC. For details on how to do this, see xref:3.7@setting-up-mmc-mule-esb-communications.adoc[Setting Up MMC-Mule ESB Communications].

To use MMC in a production environment, you need to deploy MMC to a Web application server such as Tomcat. You do this when xref:3.6@installing-the-production-version-of-mmc.adoc[installing the production version of MMC].
...
* xref:3.7@persisting-mmc-data-on-external-databases.adoc[Set up external databases] to persist MMC environment or transaction data (Business Events)
* xref:3.7@managing-mmc-users-and-roles.adoc[Manage MMC users and roles] via LDAP
...
See xref:3.6@setting-up-and-managing-users-in-mmc.adoc[Setting Up and Managing Users in MMC] for more information.

If you have an existing LDAP database to manage user data such as usernames and passwords, you can configure MMC to connect directly to the LDAP server for user management. Note that you will still need to xref:3.7@managing-mmc-users-and-roles.adoc[define user groups and permissions] using the MMC interface.

See xref:3.6@setting-up-and-managing-users-via-ldap.adoc[Setting Up and Managing Users via LDAP] for more information.
...
* xref:3.6@persisting-mmc-data-to-oracle.adoc[MMC version 3.4.1 and earlier]
* xref:3.6@persisting-mmc-data-to-oracle.adoc[MMC version 3.4.2 and later]

==== PostgresSQL

* xref:3.6@persisting-mmc-data-to-postgresql.adoc[MMC version 3.4.1 and earlier]
* xref:3.6@persisting-mmc-data-to-postgresql.adoc[MMC version 3.4.2]xref:3.6@persisting-mmc-data-to-oracle.adoc[and later]

==== MySQL

* xref:3.7@persisting-mmc-data-to-mysql.adoc[MMC version 3.4.1 and earlier]
* xref:3.7@persisting-mmc-data-to-mysql.adoc[MMC version 3.4.2]xref:3.6@persisting-mmc-data-to-oracle.adoc[and later]

==== MS SQL Server

* xref:3.6@persisting-mmc-data-to-ms-sql-server.adoc[MMC version 3.4.1 and earlier]
* xref:3.6@persisting-mmc-data-to-ms-sql-server.adoc[MMC version 3.4.2]xref:3.6@persisting-mmc-data-to-oracle.adoc[and later]

=== User Management

There are two ways of creating and configuring MMC user accounts:

* xref:3.6@setting-up-and-managing-users-in-mmc.adoc[Through the MMC interface]
* xref:3.6@setting-up-and-managing-users-via-ldap.adoc[Through LDAP]

To define user groups and assign fine-grained permissions to those groups, follow the instructions in xref:3.7@managing-mmc-users-and-roles.adoc[Managing MMC Users and Roles]. You can manage user groups and their permissions only through the MMC interface, even if you set up an LDAP server to manage user accounts.

== See Also

* Read more about xref:3.7@managing-mmc-users-and-roles.adoc[Managing MMC Users and Roles].
* Find out how to set up an xref:3.6@persisting-mmc-data-to-oracle.adoc[Oracle], xref:3.6@persisting-mmc-data-to-postgresql.adoc[PostgreSQL], xref:3.7@persisting-mmc-data-to-mysql.adoc[MySQL], or xref:3.6@persisting-mmc-data-to-ms-sql-server.adoc[MS SQL Server] database to persist your environment or transaction data.
* Learn about the  xref:3.6@architecture-of-the-mule-management-console.adoc[technical architecture of MMC].