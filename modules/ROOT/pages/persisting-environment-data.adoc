...
[WARNING]
Do not confuse MMC state info with Business Events persistence. They are two independent systems, not related to each other. Refer to a detailed description of https://docs.mulesoft.com/mule-user-guide/v/3.6/business-events[Business Events].
...
In the same directory, locate the file `jackrabbit-repo.xml` and open it for editing. Replace the appropriate parameters to use Oracle instead of the default Derby database. A full listing of the modified file is provided below; you may wish to replace your default `jackrabbit-repo.xml` with the contents of this file.
...
In the same directory, locate the file `jackrabbit-repo.xml` and open it for editing. Replace the appropriate parameters to use Postgres instead of the default Derby database. A full listing of the modified file is provided below; you may wish to replace your default `jackrabbit-repo.xml` with the contents of this file.
...
. Extract the .zip or .tar.gz installation and file, then, in the resulting directory structure, locate the file called `mysql-connector-java-<version>-bin.jar`. This is the JDBC driver itself, that you will copy to the Management Console directory structure.
